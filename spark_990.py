...
filings_directory = "/mnt/c/Users/Gautam/form_990"

def get_numfields_from_filing_link(filing_url):
    num_fields = []
    filing = ""
    try:
        with urlopen(filing_url) as resp:
            filing = resp.read()
        num_fields = get_numfields_from_filing(filing)
...
filings = sc.wholeTextFiles(filings_directory).map(lambda x: x[1])
...
#fields = filing_links.flatMap(lambda link: get_numfields_from_filing_link(link))
fields = filings.flatMap(lambda filing: get_numfields_from_filing(filing))
